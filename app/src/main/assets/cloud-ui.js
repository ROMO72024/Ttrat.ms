(function () {
  ...
  function banner(){
    ...
    const text=s.busy?'جارٍ الاتصال بالشيت…':conflicts?'هناك اختلافات تحتاج مراجعتك':overlaps?'هناك مواعيد متداخلة؛ راجعيها من إعدادات الربط':s.error?'لم تكتمل آخر محاولة؛ عملك محفوظ على الهاتف':!s.enabled?'المزامنة متوقفة · العمل المحلي مستمر':s.pending?'تعديلات محلية تنتظر المزامنة':'الربط مفعّل · آخر مزامنة '+lastDate(s.lastSync);
    return `<div class="cloud-strip ${s.error||conflicts?'cloud-warning':''}" role="status"><span class="cloud-light ${s.busy?'cloud-pulse':''}"></span><span class="grow">${e(s.accountName||'')} · ${e(text)}</span>${U.button('الربط','go-settings','ghost small','','settings')}${U.button('خروج','cloud-logout','ghost small','','')}</div>`;
  }
  function panel(){
    const s=status(),count=(s.conflicts||[]).length;
    return `<section class="panel cloud-panel"><div class="cloud-heading"><div><p class="eyebrow">${e(s.accountName||'مساحة الأخصائية')}</p><h2>مساحتك واحدة… على أكثر من هاتف</h2><p>الطلاب والخطط والجلسات والحضور تتزامن داخل مساحة الكود نفسه. التذكيرات الشخصية محلية على هذا الهاتف.</p></div><img src="logo.png" alt="شعار غرس"></div><div class="cloud-stats"><div><small>آخر مزامنة ناجحة</small><strong>${e(lastDate(s.lastSync))}</strong></div><div><small>سجلات تنتظر المزامنة</small><strong>${Number(s.pending||0).toLocaleString('ar-EG')}</strong></div><div><small>تعارضات للمراجعة</small><strong>${count.toLocaleString('ar-EG')}</strong></div></div>${s.error?`<p class="cloud-error" role="alert">${e(s.error)}</p>`:''}${s.message?`<p class="settings-note">${e(s.message)}</p>`:''}${s.busy?'<p class="settings-note">جارٍ الاتصال… يمكنك متابعة العمل المحلي.</p>':''}<div class="setting-actions cloud-actions">${s.verified&&!s.enabled?U.button(s.lastSync?'استئناف المزامنة':'رفع الأسماء الحالية وتفعيل المزامنة','cloud-enable','primary small','','upload'):''}${s.enabled?U.button('مزامنة الآن','cloud-sync','primary small','','upload')+U.button('إيقاف المزامنة','cloud-pause','ghost small','',''):''}${count?U.button('مراجعة الاختلافات','cloud-review','secondary small','','note'):''}${s.overlaps?.length?U.button('مراجعة المواعيد المتداخلة','cloud-overlaps','secondary small','','calendar'):''}${U.button('تسجيل الخروج / تبديل الأخصائية','cloud-logout','small','','')}</div><p class="settings-note">${native?'العمل المحلي مستمر دون إنترنت. نحاول المزامنة بعد التعديل وعند عودة الشبكة، وكل دقيقة أثناء فتح التطبيق. الفحص الخلفي دوري وقد يتأخر بسبب قيود Android.':'معاينة فقط: الدخول بالكود والمزامنة متاحان داخل تطبيق Android.'}</p><div class="cloud-footnote">قبل أول رفع: صدّري نسخة احتياطية، وتأكدي أن الكود يخص صاحبة الأسماء الحالية. تغيير الكود في الشيت لا يغيّر معرّف المساحة أو يمسح أسماءها. ملف الشيت يحتوي بيانات الطلاب والجلسات؛ لا تشاركيه للعامة.</div></section>`;
  }
  function review(){
